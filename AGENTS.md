...
`/setup-matt-pocock-skills` → route (`/wayfinder` | `/grill-with-docs`; `/triage` when tracker intake) → `/to-spec` → `/to-tickets` → `/implement` (Red→Green + CI-matching verification) → `/code-review` (model-invoked)
